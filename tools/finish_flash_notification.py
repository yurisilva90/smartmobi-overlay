...
assert old in s; drv.write_text(s.replace(old,new,1))
...
assert old in s; s=s.replace(old,new,1)
old='''        try { nm.notify(4103, builder.build()) } catch (_: Exception) {}'''; new='''        try { nm.notify(4103, builder.build()) } catch (_: Exception) {} finally { try { offerImage?.recycle() } catch (_: Exception) {} }'''
assert old in s; s=s.replace(old,new,1)
tr.write_text(s)
g=grad.read_text().replace('versionCode 225','versionCode 226').replace('versionName "1.3.12"','versionName "1.3.13"')
grad.write_text(g)
