...
# target only showRouteNotification signature: use rfind before body marker
...
# Não aborta a notificação só porque o parser não conseguiu endereço; o Flash ainda deve ficar salvo na central.
...
new5='''        builder.setContentTitle(titulo)\n            .setContentText(lines.firstOrNull() ?: titulo)\n        if (screenshot != null) {\n            // Mostra a própria tela capturada da oferta na notificação expandida.\n            // BigPictureStyle substitui apenas o estilo visual; o resumo textual\n            // continua disponível no contentText e nos botões de rota.\n            builder.setStyle(Notification.BigPictureStyle().bigPicture(screenshot).setSummaryText(resumo))\n        } else {\n            builder.setStyle(Notification.BigTextStyle().bigText(resumo))\n        }\n        builder'''
assert old5 in s, 'style anchor missing'
s=s.replace(old5,new5,1)

# recicla a cópia depois que NotificationManager materializa a notificação.
